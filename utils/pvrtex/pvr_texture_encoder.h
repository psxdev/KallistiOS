...
typedef enum {
	...
	PTE_FIX_MIP_OPTIONAL,	//Disable mipmaps if can't make them
} pteFixMipSizeMethod;
typedef enum {
	PTE_NORM_REGULAR,
	PTE_NORM_TEXCONV,
...
//
	...
	//Controls generation of spherical normal
	pteNormalStyle normal_style;
	...
	//Flip the image upside-down before converting
	//Basically, this puts UV coord (0,0) at the bottom left, instead of
	//the top left.
	bool flip_v;
	...
	//If NULL, palette is autogenerated
	pxlABGR8888 *palette;
	
	//This is the pixel format to report to the PVR, free from internal pseudo formats
	//like PT_YUV_TWID or PTE_BUMP. This is always <=PT_PALETTE_8B
	ptPixelFormat hw_pixel_format;
...
static inline bool pteIsPalettized(const PvrTexEncoder *pte) {
	return pte->pixel_format == PT_PALETTE_4B || pte->pixel_format == PT_PALETTE_8B;
}
